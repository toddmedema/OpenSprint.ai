...
| **Dependency tracking** (task store `blocks` relation)      | `TaskStoreService.ready()` filters to tasks whose dependencies are all resolved.                                                                                 |
...
This is the cheapest and most reliable source because the Planner already reasons about implementation details. The orchestrator stores these annotations on the task as structured metadata (e.g. labels).
...
4. **Extend the Planner output schema** to include `files` predictions. Update the Planner prompt template and the orchestrator's task-creation logic to store file metadata on tasks (e.g. labels).
5. **Implement `FileScopeAnalyzer`** with all three prediction layers. Add unit tests with known Plan structures.
6. **Record actual files changed** after each task completes (from `getChangedFiles()`). Store as task metadata (e.g. labels) for future inference.
...
| File scope stored as task metadata (e.g. labels)           | Natural location; survives restarts; accessible to context assembler                                   |
