...
const pgResult = await createTestPostgresClient();
const postgresOk = !!pgResult;

describe.skipIf(!postgresOk)("BehaviorVersionStore", () => {
  let client: DbClient;

  beforeAll(async () => {
    client = pgResult!.client;
  ...
  it("promoteToActive persists version row and active pointer", async () => {
  ...
  it("setActivePromoted updates pointer only", async () => {
    const store = new BehaviorVersionStore(() => client);
  ...
  it("resolveActiveForExecute returns template from store", async () => {
    const store = new BehaviorVersionStore(() => client);
  ...
  it("readActivePromotedReplayBinding reads pointer and template from store", async () => {
    const store = new BehaviorVersionStore(() => client);
  ...
  it("readActivePromotedReplayBinding returns null when no project_behavior_state row", async () => {
    const store = new BehaviorVersionStore(() => client);
