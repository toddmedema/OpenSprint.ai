...
`,
  ...
  it("rehydrates task worktree before npm ci repair and skips node_modules relink", async () => {
    const repoPath = await makeTempWorktree({ lint: "eslint ." });
    const worktreePath = await makeTempWorktree({ lint: "eslint ." });
    let lintAttempts = 0;
    const runCommand = vi.fn(
      async (spec: { command: string; args?: string[] }, options: { cwd: string }) => {
        const label = commandLabel(spec);
        if (label === "git rev-parse --verify HEAD" || label === "npm ls --depth=0") {
          return makeCommandResult(spec, options.cwd);
        }
        ...
        if (label === "git reset --hard HEAD" || label === "git clean -fd" || label === "npm ci") {
          return makeCommandResult(spec, options.cwd);
        }
        return makeCommandResult(spec, options.cwd);
      }
