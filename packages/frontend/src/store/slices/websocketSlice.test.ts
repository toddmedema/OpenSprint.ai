...
const mockHilRequest: HilRequestEvent = {
  ...
  options: [{ id: "opt-1", label: "Approve", description: "Approve the change" }],
  blocking: true,
};
...
const mockHilNotification: HilRequestEvent = {
  ...
  options: [{ id: "opt-2", label: "OK", description: "Acknowledge" }],
  blocking: false,
};
...
describe("websocketSlice", () => {
  ...
  describe("setHilNotification", () => {
    it("stores HIL notification event", () => {
      ...
      expect(store.getState().websocket.hilNotification).toEqual(mockHilNotification);
    });
